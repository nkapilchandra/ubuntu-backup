...
var track = document.getElementById('bound');
...
function boundary(i,j) {
    if(i!=size-1){
    if(s[i][j]!=s[i+1][j]){
    ctx.moveTo(squareWidth*(i+1),squareWidth*j);
    ctx.lineTo(squareWidth*(i+1),squareWidth*(j+1));
    ctx.stroke();
    }
  }
  ...
  if(j!=size-1){
    if(s[i][j]!=s[i][j+1]){
    ctx.moveTo(squareWidth*(i),squareWidth*(j+1));
    ctx.lineTo(squareWidth*(i+1),squareWidth*(j+1));
    ctx.stroke();
    }
  }
